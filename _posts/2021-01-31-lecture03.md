...
How does this help? The term $v_t$ is called the *momentum* and it is a running average of the gradients. It is updated as we saw above, and the intuition is that it is a weighted average of the current gradient and the previous momentum. So it is a kind of "memory" of the previous gradients. The update equation for $v_t$ can be expanded out as follows:


$$ v_t = \beta g_t + (1 - \beta) \beta g_{t-1} + (1 - \beta)^2 \beta g_{t-2} + \ldots $$
$$ v_t = \sum_{i=0}^{t-1} (1 - \beta)^i \beta g_{t-i} $$
$$ v_t = \sum_{i=0}^{t-1} \mu^i \beta g_{t-i} $$

where $\mu = 1 - \beta$. So the momentum is a weighted sum of the gradients, with the weights decaying exponentially with time. This is a very common pattern in machine learning, and is called an *exponentially weighted moving average* (EWMA).
...
A typical choice of $\beta$: 0.9 or 0.99. This is called *momentum-accelerated gradient descent*. Interestingly, this simple change effects a *quadratic* acceleration to gradient descent for linear models (i.e., the number of iterations needed to reach a certain training error is the square root of the number required by those used in gradient descent.)
Using 0.9 is a good default choice and is used in most deep learning frameworks. This provides a good balance between the current gradient and the previous momentum by considering the last 10 iterations. If we use a larger value of $\beta$ (say, 0.999) then we are considering the last 1000 iterations, which is too much memory and can lead to oscillations.
